...
export class ExceptionError implements SourceError {
  ...
  constructor(public error: Error, public location: ast.SourceLocation) {}
...
export class MaximumStackLimitExceeded extends RuntimeSourceError {
  ...
  constructor(node: ast.Node, private calls: ast.CallExpression[]) {
    super(node)
  }

  public explain() {
    const repr = (call: ast.CallExpression) => generate(call, { generator: this.customGenerator })
    return (
...
export class CallingNonFunctionValue extends RuntimeSourceError {
  constructor(private callee: Value, private node: ast.Node) {
    super(node)
  }
  ...
  public elaborate() {
    ...
    const callArgs = (this.node as ast.CallExpression).arguments
...
export class UndefinedVariable extends RuntimeSourceError {
  constructor(public name: string, node: ast.Node) {
    super(node)
  }
...
export class InvalidNumberOfArguments extends RuntimeSourceError {
  ...
  constructor(node: ast.Node, private expected: number, private got: number) {
    super(node)
    this.calleeStr = generate((node as ast.CallExpression).callee)
  }
...
export class VariableRedeclaration extends RuntimeSourceError {
  constructor(private node: ast.Node, private name: string, private writable?: boolean) {
    super(node)
  }
  ...
  public elaborate() {
    if (this.writable === true) {
      ...
      if (this.node.type === 'FunctionDeclaration') {
        initStr =
          '(' + (this.node as ast.FunctionDeclaration).params.map(generate).join(',') + ') => {...'
      } else if (this.node.type === 'VariableDeclaration') {
        initStr = generate((this.node as ast.VariableDeclaration).declarations[0].init)
      }
...
export class ConstAssignment extends RuntimeSourceError {
  constructor(node: ast.Node, private name: string) {
    super(node)
  }
...
export class GetPropertyError extends RuntimeSourceError {
  constructor(node: ast.Node, private obj: Value, private prop: string) {
    super(node)
  }
